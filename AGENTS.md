...
- Idle cost control: tenants that take no ingress traffic for an hour are scaled to zero
  ...
  check-out. Manually: `scripts/tenant-scale.sh <ID> down|up`. **Persistent tenants are
  exempt** — the idle scan skips anything without a control-table item, so a standing
  per-person environment answers its URL with no wake step, and reserves its requests
  (and ~15 pod IPs on `full`) permanently.
