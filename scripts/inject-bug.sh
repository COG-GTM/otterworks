...
if [ "${SCENARIO}" = "file-upload-always-fails" ]; then
  ...
  log "(a redeploy clears the injection either way: the chart and the service image both"
  log "leave the variable off, so uploads recover on the next deploy-tenant.sh run.)"
  exit 0
fi
