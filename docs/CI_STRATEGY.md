...
3. **Report service Trivy exclusion** — `services/report-service` is still in Trivy's `--skip-dirs` from when it was intentionally outdated; now that it runs on Java 17 / Spring Boot 3.2, that exclusion should be removed so the service is scanned again.
