...
| legacy-portal | Java 17 (Maven wrapper) | `./mvnw verify` | Passing |
...
3. **Legacy service isolation** — Report service is still skipped by Trivy. It was excluded while intentionally outdated; now that it runs on Java 17 / Spring Boot 3.2 the exclusion can be dropped once the first full scan is triaged.
