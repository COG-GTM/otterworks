...
1. Confirm the error in file-service logs:
   ...
   ```
2. Check the bucket file-service is configured to write to, and that it exists:
   ```
   kubectl get deploy/file-service -n otterworks -o jsonpath='{.spec.template.spec.containers[0].env}' | tr ',' '\n' | grep -i s3_bucket
