...
// envKeys is every variable Load reads. Tests clear them so an inherited
...
// clearEnv unsets every configuration variable for the duration of the test.
// getEnv uses os.LookupEnv, so an empty value still beats the default: the key
// has to be absent, and t.Setenv is only used to register the restore.
...
func TestLoad_EnvOverrides(t *testing.T) {
	...
	t.Setenv("CORS_ALLOWED_METHODS", "GET,HEAD")
	t.Setenv("CORS_ALLOWED_HEADERS", "Accept")
	...
	assert.Equal(t, []string{"GET", "HEAD"}, cfg.CORSAllowedMethods)
	assert.Equal(t, []string{"Accept"}, cfg.CORSAllowedHeaders)
...
func TestGetEnvHelpers(t *testing.T) {
	...
	t.Run("getEnv", func(t *testing.T) {
		assert.Equal(t, "fallback", getEnv("PORT", "fallback"))
		t.Setenv("PORT", "set")
		assert.Equal(t, "set", getEnv("PORT", "fallback"))
	})

	t.Run("getEnvInt", func(t *testing.T) {
		assert.Equal(t, 3, getEnvInt("CORS_MAX_AGE", 3))
		t.Setenv("CORS_MAX_AGE", "42")
		assert.Equal(t, 42, getEnvInt("CORS_MAX_AGE", 3))
		t.Setenv("CORS_MAX_AGE", "nope")
		assert.Equal(t, 3, getEnvInt("CORS_MAX_AGE", 3))
	})

	t.Run("getEnvFloat", func(t *testing.T) {
		assert.InDelta(t, 1.5, getEnvFloat("CB_FAILURE_RATIO", 1.5), 1e-9)
		t.Setenv("CB_FAILURE_RATIO", "0.75")
		assert.InDelta(t, 0.75, getEnvFloat("CB_FAILURE_RATIO", 1.5), 1e-9)
		t.Setenv("CB_FAILURE_RATIO", "nope")
		assert.InDelta(t, 1.5, getEnvFloat("CB_FAILURE_RATIO", 1.5), 1e-9)
	})

	t.Run("getEnvSlice", func(t *testing.T) {
		fallback := []string{"a", "b"}
		assert.Equal(t, fallback, getEnvSlice("CORS_ALLOWED_ORIGINS", fallback))
		t.Setenv("CORS_ALLOWED_ORIGINS", "x,y,z")
		assert.Equal(t, []string{"x", "y", "z"}, getEnvSlice("CORS_ALLOWED_ORIGINS", fallback))
		t.Setenv("CORS_ALLOWED_ORIGINS", "")
		assert.Equal(t, fallback, getEnvSlice("CORS_ALLOWED_ORIGINS", fallback))
	})
}
