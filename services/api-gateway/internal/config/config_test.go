...
// clearConfigEnv unsets every variable Load reads so a test starts from a known
// state. getEnv uses os.LookupEnv, so an *empty* variable still beats the
// default -- the variable has to be unset, not set to "".
func clearConfigEnv(t *testing.T) {
	...
	for _, key := range []string{
		...
		// t.Setenv registers the restore hook, then unset for the test body.
		t.Setenv(key, "")
...
func TestLoad_MalformedValuesFallBackToDefaults(t *testing.T) {
	...
	// An empty slice variable is treated as "unset" by getEnvSlice.
...
func TestValidate(t *testing.T) {
	tests := []struct {
		...
		{name: "missing secret", secret: "", wantErr: "JWT_SECRET"},
		{name: "present secret", secret: "s3cret"},
	}
...
func TestGetEnvHelpers(t *testing.T) {
	t.Run("getEnv falls back when unset", func(t *testing.T) {
		t.Setenv("OW_TEST_STRING", "x")
		require.NoError(t, os.Unsetenv("OW_TEST_STRING"))
		assert.Equal(t, "fallback", getEnv("OW_TEST_STRING", "fallback"))
	})

	t.Run("getEnv returns an empty value over the fallback", func(t *testing.T) {
		t.Setenv("OW_TEST_STRING", "")
		assert.Equal(t, "", getEnv("OW_TEST_STRING", "fallback"))
	})

	t.Run("getEnvInt parses and falls back", func(t *testing.T) {
		t.Setenv("OW_TEST_INT", "42")
		assert.Equal(t, 42, getEnvInt("OW_TEST_INT", 1))
		t.Setenv("OW_TEST_INT", "nope")
		assert.Equal(t, 1, getEnvInt("OW_TEST_INT", 1))
		require.NoError(t, os.Unsetenv("OW_TEST_INT"))
		assert.Equal(t, 1, getEnvInt("OW_TEST_INT", 1))
	})

	t.Run("getEnvFloat parses and falls back", func(t *testing.T) {
		t.Setenv("OW_TEST_FLOAT", "1.5")
		assert.InDelta(t, 1.5, getEnvFloat("OW_TEST_FLOAT", 0.5), 1e-9)
		t.Setenv("OW_TEST_FLOAT", "nope")
		assert.InDelta(t, 0.5, getEnvFloat("OW_TEST_FLOAT", 0.5), 1e-9)
		require.NoError(t, os.Unsetenv("OW_TEST_FLOAT"))
		assert.InDelta(t, 0.5, getEnvFloat("OW_TEST_FLOAT", 0.5), 1e-9)
	})

	t.Run("getEnvSlice splits on commas and falls back when empty", func(t *testing.T) {
		t.Setenv("OW_TEST_SLICE", "a,b,c")
		assert.Equal(t, []string{"a", "b", "c"}, getEnvSlice("OW_TEST_SLICE", []string{"z"}))
		t.Setenv("OW_TEST_SLICE", "")
		assert.Equal(t, []string{"z"}, getEnvSlice("OW_TEST_SLICE", []string{"z"}))
		require.NoError(t, os.Unsetenv("OW_TEST_SLICE"))
		assert.Equal(t, []string{"z"}, getEnvSlice("OW_TEST_SLICE", []string{"z"}))
	})
}
