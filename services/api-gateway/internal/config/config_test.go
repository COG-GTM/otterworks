...
// clearConfigEnv removes every variable Load reads so a test observes the
// documented defaults. getEnv* use os.LookupEnv, so an *empty* variable still
// wins over the default: unset instead of setting to "".
func clearConfigEnv(t *testing.T) {
	...
	for _, key := range []string{
		...
		// t.Setenv registers the restore hook; Unsetenv then removes the variable
...
func TestLoad_MalformedValuesFallBackToDefaults(t *testing.T) {
	...
	// getEnvSlice treats an empty value as "unset" and falls back.
...
func TestLoad_EmptyStringOverridesStringDefault(t *testing.T) {
	clearConfigEnv(t)

	// getEnv uses LookupEnv, so an explicitly empty variable wins over the default.
	t.Setenv("PORT", "")

	assert.Equal(t, "", Load().Port)
}
