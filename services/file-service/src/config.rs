...
pub struct ServerConfig {
    ...
    /// first listing, so share flows are demoable on a fresh tenant.
...
mod tests {
    ...
    #[test]
    fn image_does_not_enable_upload_failure() {
        let dockerfile = include_str!("../Dockerfile");
        assert!(
            !dockerfile.contains("FILE_UPLOAD_ALWAYS_FAIL"),
            "the shipped image must not force uploads to a nonexistent bucket"
        );
    }
}
