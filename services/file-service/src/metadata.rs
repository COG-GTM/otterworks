...
impl MetadataClient {
    ...
    pub async fn put_share(&self, share: &FileShare) -> Result<(), ServiceError> {
        ...
        if let Some(email) = &share.invitee_email {
            item.insert("invitee_email".into(), AttributeValue::S(email.clone()));
        }
...
fn parse_file_share(
    ...
    // Records written before invite support carry neither attribute.
    ...
    Ok(FileShare {
        ...
        invitee_email: get_optional_s(item, "invitee_email"),
        status,
        permission,
...
mod tests {
    ...
    fn test_parse_file_share() {
        ...
        // Records written before invites existed have no status attribute
        assert_eq!(share.status, ShareStatus::Active);
        assert!(share.invitee_email.is_none());
    }
    ...
    fn test_parse_pending_invite_share() {
        ...
        assert_eq!(share.invitee_email.as_deref(), Some("outside@example.com"));
    }
