...
mod tests {
    ...
    fn body_of(err: &ServiceError) -> ErrorResponse {
        let bytes = err.error_response().into_body().try_into_bytes().unwrap();
        serde_json::from_slice::<serde_json::Value>(&bytes)
            .map(|v| ErrorResponse {
                error: v["error"].as_str().unwrap().to_string(),
    ...
    #[test]
    fn maps_every_variant_to_its_status_and_error_type() {
        let cases = [
            (
    ...
    fn error_response_displays_as_type_and_message() {
        ...
        assert!(format!("{resp:?}").contains("bad_request"));
    }
}
