...
public class Report {
    ...
    @Schema(description = "Unique report identifier", readOnly = true)
    private Long id;

    @NotNull
    @Column(name = "report_name", nullable = false)
    @Schema(description = "Human-readable report name", required = true)
    private String reportName;

    @NotNull
    @Enumerated(EnumType.STRING)
    @Column(name = "category", nullable = false)
    @Schema(description = "Report category", required = true)
    private ReportCategory category;

    @NotNull
    @Enumerated(EnumType.STRING)
    @Column(name = "report_type", nullable = false)
    @Schema(description = "Output format: PDF, CSV, or EXCEL", required = true)
    private ReportType reportType;

    @NotNull
    @Enumerated(EnumType.STRING)
    @Column(name = "status", nullable = false)
    @Schema(description = "Current generation status", readOnly = true)
    ...
    @Schema(description = "When the report was requested", readOnly = true)
    private Date createdAt;

    // LEGACY: java.util.Date instead of java.time.Instant
    @Temporal(TemporalType.TIMESTAMP)
    @Column(name = "completed_at")
    @Schema(description = "When the report finished generating", readOnly = true)
    ...
    @Lob
    // Hibernate 6 maps @Lob String to PostgreSQL oid by default; this keeps it a text column.
