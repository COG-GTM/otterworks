 ...
 * - Mutable POJO with setters (target: record)
 */
@Schema(description = "Request to generate a new report")
public class ReportRequest {

    @NotBlank(message = "Report name is required")
    @Schema(description = "Human-readable name for the report", required = true, example = "Monthly Usage Report")
    private String reportName;

    @NotNull(message = "Report category is required")
    @Schema(description = "Category of data to include", required = true)
    private ReportCategory category;

    @NotNull(message = "Report type is required")
    @Schema(description = "Output format", required = true, example = "PDF")
    private ReportType reportType;

    @NotBlank(message = "Requester ID is required")
    @Schema(description = "User ID requesting the report", required = true)
