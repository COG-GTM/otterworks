...
describe('loadConfig', () => {
  ...
  it('reads every value from the environment when it is provided', () => {
    process.env.HTTP_PORT = '9100';
    process.env.REDIS_HOST = 'redis.internal';
    process.env.REDIS_PORT = '6380';
    process.env.REDIS_PASSWORD = 'hunter2';
    process.env.REDIS_DB = '4';
    process.env.REDIS_KEY_PREFIX = 'tenant:collab:';
    ...
    expect(cfg.redis).toEqual({
      ...
      password: 'hunter2',
      db: 4,
      keyPrefix: 'tenant:collab:',
    });
    expect(cfg.jwt).toEqual({ secret: 'prod-secret', issuer: 'issuer.test' });
  ...
  it('treats an empty REDIS_PASSWORD as absent', () => {
