...
- Point file-service at a bucket that does not exist and restart it:
  `S3_BUCKET=otterworks-does-not-exist docker compose -f docker-compose.infra.yml -f docker-compose.yml up -d file-service`
  (revert by restarting without the override). S3 then returns `NoSuchBucket` → 500 storage_error.
- Alternative: delete the bucket in LocalStack (`docker exec otterworks-localstack awslocal s3 rb s3://otterworks-files --force`).
