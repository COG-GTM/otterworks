...
- file-service always uploads to its configured bucket, so induce the failure through
  configuration: point `S3_BUCKET` at a bucket that does not exist (the `file-bad-bucket`
  scenario in `scripts/bug-catalog.yaml`) and restart the service → 500 storage_error.
- The Redis flag `chaos:file-service:upload_s3_error` and the admin dashboard's "Break File
  Uploads" control no longer affect the upload path.
