...
(`FILE_SHARE_EVENT_ALWAYS_FAIL`, PR #209). Note that a similar upload switch
(`FILE_UPLOAD_ALWAYS_FAIL`) was removed after it caused a live upload outage on
the coggtm tenant — scope new switches so they cannot break a tenant's core path
unattended. Companion skill `purposeful-failure-devin-trigger` covers how the
...
- One env var per failure, `<SERVICE>_<THING>_ALWAYS_FAIL`, parsed in the
  ...
  agreement with the image ENVs (see the `FILE_SHARE_EVENT_ALWAYS_FAIL` comment
  in `infrastructure/helm/file-service/values.yaml`) so the two never contradict —
