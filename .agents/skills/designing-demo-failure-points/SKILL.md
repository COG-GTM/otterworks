...
(`FILE_SHARE_EVENT_ALWAYS_FAIL`, PR #209). For a storage failure, misconfigure
the resource itself (the `file-bad-bucket` scenario) rather than adding a switch
that redirects writes away from the configured bucket. Companion skill `purposeful-failure-devin-trigger` covers how the
