...
- In file-service, an env switch read in `src/config.rs` (default off) makes
  `handlers.rs::upload_bucket` return the nonexistent bucket
  `otterworks-files-chaos-nonexistent`, so S3 returns `NoSuchBucket` and the
