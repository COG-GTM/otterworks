...
- Add a `<SERVICE>_<OPERATION>_ALWAYS_FAIL`-style env switch to the target service's
  config (e.g. parsed in the Rust services' `src/config.rs`, default off) that points
  the operation at a nonexistent backing resource, so the SDK returns a genuine error
  (e.g. S3 `NoSuchBucket`) and the request 500s. Leave any existing Redis chaos checks
  intact for the other scenarios.
- Do not build this switch into file-service uploads: that path is deliberately kept
  free of failure switches after the tenant upload outage (see PR #138).
...
- The reliable fork-side mechanism: **bake it into the service image**:
  ...
  ENV <SERVICE>_<OPERATION>_ALWAYS_FAIL=true
  ```
