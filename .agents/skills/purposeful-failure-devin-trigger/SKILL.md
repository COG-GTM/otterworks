...
- In file-service, `FILE_SHARE_EVENT_ALWAYS_FAIL` (parsed in `src/config.rs`,
  default off) publishes the `file_shared` event to a nonexistent SNS topic, so
  AWS returns `NotFound` and the share request 500s. The Redis chaos flags stay
  intact for other scenarios.
