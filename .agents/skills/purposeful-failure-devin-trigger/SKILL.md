...
- Shape: parse a `<SERVICE>_<BEHAVIOUR>` bool in the service's config module
  (default off) and, when it is on, point the operation at a nonexistent resource
  (e.g. a bucket that does not exist) so the SDK's own error surfaces as a 5xx.
  Note file-service no longer carries such a switch: the upload path always writes
  to the configured bucket, so a file-upload demo needs a config-level override
  instead (`file-bad-bucket` in `scripts/bug-catalog.yaml` sets a bad `S3_BUCKET`).
